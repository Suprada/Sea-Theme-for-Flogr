...
class Flogr_Set extends Flogr_Page {
    ...
    function get_set_list( $userId = null, $before = '', $inner = '', $after = '' ) {
        ...
        foreach( $setList['photoset'] as $photoset ) {
            if ( !FLOGR_PHOTOSETS_INCLUDE || stristr(FLOGR_PHOTOSETS_INCLUDE, $photoset['title']) ) {
                $photosetList .= $this->get_set_link($photoset, $this->get_primary_photo_img($photoset, "small"));
            }
        }
        return $photosetList;
    }
    ...
    function get_primary_photo_img( $set, $size = null ) {
        ...
        return "<div id='format_thumbnail'><img class='thumbnail' title='{$title}' rel='{$desc}' src='http://farm" . $set['farm'] . ".static.flickr.com/" . $set['server'] . "/" . $set['primary'] . "_" . $set['secret'] . $sizes[$size] . ".jpg'/><p>$title</p></div>";
    }
    ...
    function get_set_link( $set, $inner = '' ) {
        ...
        $desc = htmlspecialchars($set['description'], ENT_QUOTES);
        $id = $set['id'];
        $url = $this->phpFlickr->buildPhotoURL($set['id'], "small");
